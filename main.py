...
@commands.bot_has_role(8)
def seconds_until(hours, minutes):
	today = datetime.now(pytz.timezone('Asia/Kolkata'))
	given_time = time(hours, minutes)
	now = datetime.now(pytz.timezone('Asia/Kolkata'))
	now = now.replace(tzinfo=pytz.timezone('Asia/Kolkata'))
	future_exec = datetime.combine(now, given_time)
	future_exec = future_exec.replace(tzinfo=pytz.timezone('Asia/Kolkata'))
	if (
	    future_exec - now
	).days < 0:  # If we are past the execution, it will take place tomorrow
		future_exec = datetime.combine(now + timedelta(days=1),
		                               given_time)  # days always >= 0
		future_exec = future_exec.replace(tzinfo=pytz.timezone('Asia/Kolkata'))
...
async def TodayBday():
    ...
    """
	waitTime = seconds_until(00, 00)
	await LogPrint("BDAY BOT : Seconds until 00:00 IST : " + str(waitTime) +
	               "\nSo Sleeping until then 😪😴🛌")
